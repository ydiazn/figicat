...
Player.prototype.updateMovement = function(cursors) {
    ...
    if (this.isFrozen) {
        ...
        if (gameState != GameState.PrincipalMenu && cursors.left.isDown) {
        ...
        if (gameState != GameState.PrincipalMenu && cursors.right.isDown) {
        ...
        if (gameState != GameState.PrincipalMenu && (cursors.up.isDown || jumpButton.isDown)) {            
            this.body.acceleration.y = -1500;
        }
        
        if (gameState != GameState.PrincipalMenu && cursors.down.isDown) {
            this.body.acceleration.y = 1500;
        }
