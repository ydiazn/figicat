...
function onKeyDown(keyEvent) {
    ...
    if (keyEvent.keyCode == 13){
        ...
        else if (this.gameover.visible && this.goverinstructions.visible) {
            this.gameover_input_handler();
        }
    }
    else if(keyEvent.keyCode == 27){
        gameState = GameState.PrincipalMenu;
        this.principalmenu_input_handler(); 
    }        
};
...
UIManager.initGameOverScreen = function() {
    ...
    var gameOverLabel = game.add.bitmapText(game.world.centerX - 140, game.world.centerY - 15, 'carrier_command', "JUEGO TERMINADO", 12*2);
    gameover.add(gameOverLabel);

    var instructionsStr = "PRESIONA [ENTRAR]";
...
UIManager.initMainMenu = function() {
    ...
    menug.drawRect(0, game.world.centerY - 80, game.width, 190);
    menug.endFill();
    mainmenu.add(menug);

    var menuCaption = game.add.bitmapText(game.world.centerX - 320, game.world.centerY - 60, 'carrier_command', "Buscador de Diamantes", 12 *2);
    mainmenu.add(menuCaption);

    var instructionsStr1 = "MOVERSE: Izquierda,Derecha, Arriba, Abajo";
    var instructions1 = game.add.bitmapText(game.world.centerX - 330, game.world.centerY, 'carrier_command', instructionsStr1, 12);
    mainmenu.add(instructions1);

    var instructionsStr2 = "PRESIONA [ENTRAR] PARA EMPEZAR";
    var instructions2 = game.add.bitmapText(game.world.centerX - 200, game.world.centerY + 40, 'carrier_command', instructionsStr2, 12);
    mainmenu.add(instructions2);
    
    var instructionsStr3 = "PRESIONA [ESCAPE] PARA IR AL MENU";
    var instructions3 = game.add.bitmapText(game.world.centerX - 200, game.world.centerY + 80, 'carrier_command', instructionsStr3, 12);
    mainmenu.add(instructions3);
...
UIManager.initGameHUD = function() {
    ...
    var levelLabel = game.add.bitmapText(game.world.centerX - 60, 8, 'carrier_command', "NIVEL 1", 16);
...
UIManager.setLevel = function(level) {
	this.levelLabel.setText("NIVEL "+level);
}
...
UIManager.setUIEventHandler = function(eventName, func) {
	...
	else if (eventName == "mainmenu_input") {
		this.mainmenu_input_handler = func;
	}
        else if (eventName == "principalmenu_input") {
		this.principalmenu_input_handler = func;
	}
