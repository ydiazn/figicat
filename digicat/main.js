...
StorageManager.init();
currentplayer = StorageManager.firstUser();

//Game
var game = new Phaser.Game(800, 600, Phaser.AUTO, 'game', { preload: preload, create: create, update: update, render: render, preRender: preRender });

function preload() {

    game.load.spritesheet('tilemap', 'assets/ground_1x1_bw2.png?1', 32, 32);
    ...
    game.load.script('NaiveAnaglyph', 'filters/NaiveAnaglyph.js');
    game.load.script('Anaglyph', 'filters/Anaglyph.js');
}

function create() {

    //Dichotpic filter    
    DichopticManager.init(game, currentplayer);
    
    //DichopticManager.colorAnaglyph();
...
function preRender(){
    if(DichopticManager.renderTexture){
        if(currentplayer.medical.amblyopicEye == StorageManager.eyes.right){
            EnemyManager.enemies.renderable = true;
            EnemyManager.enemies.setAll("renderable", true);
            
        }
        else{
            EnemyManager.enemies.renderable = false;
            EnemyManager.enemies.setAll("renderable", false);
        }
        DichopticManager.render(game);
        EnemyManager.enemies.setAll("renderable", !EnemyManager.enemies.renderable);
        EnemyManager.enemies.renderable = !EnemyManager.enemies.renderable;
    }
}
